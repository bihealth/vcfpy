...
def test_call_init_with_gt():
    ...
    expected_allele_0 = 0
    expected_allele_1 = 1
    assert call.gt_alleles is not None
...
def test_is_phased_mixed():
    call = record.Call("sample", {"GT": "0/1|2"})
    assert call.is_phased is True
...
def test_gt_bases_0_0():
    call = record.Call("sample", {"GT": "0|0"})
    build_rec([call])
...
def test_gt_bases_0_2():
    call = record.Call("sample", {"GT": "0|2"})
    build_rec([call])
...
def test_gt_type_filtered_empty():
    call = record.Call("sample", {"GT": "1/1", "FT": []})
    assert not call.is_filtered()
